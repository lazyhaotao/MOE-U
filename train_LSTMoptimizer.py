...
def fit(optimizer_network, meta_optimizer, optimizee_obj_function, optimizee_network,
    ...
    return losses_list
def main_loop(OptimizerNetwork, optimizee_obj_function, optimizee_network, preprocessing = False,
        epochs = 20, iterations_to_optimize = 100, iterations_to_unroll = 20, 
        n_tests = 10, lr = 0.001, out_mul = 1.0):
    ...
    for eop_epo in range(epochs):
        #print('epotion:',eop_epo,',iteration:',itera_itera)
        train_loss_list = fit(optimizer_network, meta_optimizer, optimizee_obj_function, optimizee_network,
            iterations_to_optimize, iterations_to_unroll, out_mul,
            should_train = True)
        current_loss = np.mean(np.sum(train_loss_list))
        print(current_loss)
        ...
        #        ]))
        # print(current_loss)
        # every 200 episodes to save the model
        if (eop_epo + 1) % 100 ==0:
            #best_net = copy.deepcopy(optimizer_network.state_dict())
            torch.save(optimizer_network.state_dict(), '../LstmOptimizerPth/LSTMO_FullyC_PaviaC_lr'+str(lr)+'episode'+str(eop_epo)+'.pth')
    return current_loss


if __name__ == '__main__':
    USE_CUDA  = True
    main_loop(OptimizerNetwork, HSIUtil, GenericNeuralNetforPaviaC, lr=0.01, out_mul=0.1, preprocessing=True, n_tests=5, epochs=700)