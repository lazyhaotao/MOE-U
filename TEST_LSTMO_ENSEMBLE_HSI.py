...
# our update integration algorithm
class ensemble_strategy_next():
    ...
    def look_next(self, param_list, optimizee_obj_function, optimizee_network):
        for i in range(len(param_list)):
            ...
            current_loss = optimizee(optimizee_obj_function)[0].data.cpu().numpy()
            self.momentum_list[i] = self.momentum_list[i] * self.beta + ((current_loss - self.last_loss)) * (
                        1 - self.beta)
...
def fit_ensemble_test(optimizer_network, meta_optimizer, optimizee_obj_function, optimizee_network,
                      iterations_to_optimize, iterations_to_unroll, out_mul, ensemble_strategy_name,
                      seed, n_tests=10, SAMPLE_SIZE=10, should_train = False, ensemble_num=3, pth_list=[], ensemble_beta=0.1):
    optimizer_list = []
    ...
    for run in range(n_tests):
        ...
        optimizee.train()
        ensemble_strategy = ensemble_strategy_name(beta=ensemble_beta, ensemble_num=ensemble_num)
        ...
        for iteration in range(iterations_to_optimize):
            ...
            with torch.no_grad():
                for name, param in optimizee.all_named_parameters():
                    ...
                    offset += current_size
                final_result_params = ensemble_strategy.look_next(param_list, optimizee_obj, optimizee_network)
...
if __name__ == '__main__':
    ...
    ensemble_pth = ensemble_pth = ensemble_pth = [
                    ' ',
                    ' ',
                    ' ',
                    ]
    ...
    fit_ensemble_test(OptimizerNetwork, None, HSIUtil, GeneralFullyCTest, 50, 1, out_mul = 0.1,
                      ensemble_strategy_name = ensemble_strategy_next, seed = args.random_seed, should_train = False, n_tests=10,
                      ensemble_num=len(ensemble_pth), pth_list=ensemble_pth, ensemble_beta=0.1)
